...
def resolve_target_symbol_file(
    ...
    dbg_print(f"Configured symbol_file: {configured_symbol_file}", Severity.INFO, "symbols", "resolver", "-")
    dbg_print(f"Target footprint dir: {target_footprint_dir}", Severity.INFO, "symbols", "resolver", "-")

    if not target_footprint_dir.exists() or not target_footprint_dir.is_dir():
        dbg_print("Target footprint directory does not exist or is not a directory.", Severity.INFO, "symbols", "resolver", "-")
    ...
    if configured_symbol_file:
        ...
        if configured_path.exists():
            dbg_print(f"Using configured symbol file: {configured_path}", Severity.INFO, "symbols", "resolver", "-")
            return (
                configured_path,
                "configured symbol file exists",
            )

        dbg_print(f"Configured symbol file does not exist: {configured_path}", Severity.INFO, "symbols", "resolver", "-")
    else:
        configured_path = None
        dbg_print("No configured symbol_file value found.", Severity.INFO, "symbols", "resolver", "-")
    ...
    dbg_print(f"All symbol file matches found: {[path.name for path in all_symbol_matches]}", Severity.INFO, "symbols", "resolver", "-")
    dbg_print(f"Active symbol file matches after backup filter: {[path.name for path in symbol_matches]}", Severity.INFO, "symbols", "resolver", "-")
