...
def extract_zip_to_temp(zip_path: Path) -> Path:
    ...
    debug_print("zip", "")
    debug_print("zip", f"Extracting ZIP: {zip_path}")
    debug_print("zip", f"Temporary extract folder: {temp_root}")
...
def print_import_file_summary(found_files: dict, extract_root: Path) -> None:
    ...
    """
    debug_print("zip", "")
    debug_print("zip", "Detected files:")
    ...
    for label, key in categories:
        ...
        debug_print("zip",  "")
        debug_print("zip", f"{label}: {len(files)}")
